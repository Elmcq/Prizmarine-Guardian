...
export default {
 ...
 ban: (value) => moderationText({ userId: value.userId, action: 'TempBan', ruleId: rule.id, ruleTitle: rule.title, reason: rule.description, moderatorId: ctx.authorId, durationMs: duration }),
 } : null;
 ...
 ctx.groupId,
 moderationText({ userId: target, action: 'TempBan', reason, moderatorId: ctx.authorId, durationMs: duration }),
 [target, ctx.authorId],
 );
 }
 },
